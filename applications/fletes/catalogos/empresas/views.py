...
class RegistrarEmpresas(LoginAndActiveMixin, PermisoFletes, CreateView):
    model = NomEmpresas
    form_class = forms.FormularioEmpresas
    template_name = 'fletes/catalogos/empresas/registro_empresas.html'
    success_url = reverse_lazy('fletes:catalogos:empresas:lista')
...
class ListaEmpresas(LoginAndActiveMixin, PermisoFletes, ListView):
    model = NomEmpresas
    template_name = 'fletes/catalogos/empresas/empresas.html'
    login_url = reverse_lazy('users_app:user_login')
    ...
    def get_context_data(self, **kwargs):
        ...
        if empresa_buscada is not None:
            context['lista_empresas'] = NomEmpresas.objects.filter(activo=True, nombre__icontains=empresa_buscada)
        else:
            context['lista_empresas'] = NomEmpresas.objects.filter(activo=True)
        return context
...
class ConsultarEmpresas(LoginAndActiveMixin, PermisoFletes, DetailView):
    model = NomEmpresas
    template_name = 'fletes/catalogos/empresas/consulta_empresas.html'
    login_url = reverse_lazy('users_app:user_login')
...
class ActualizarEmpresas(LoginAndActiveMixin, PermisoFletes, UpdateView):
    model = NomEmpresas
    form_class = forms.FormularioEmpresas
    template_name = 'fletes/catalogos/empresas/registro_empresas.html'
    success_url = reverse_lazy('fletes:catalogos:empresas:lista')
...
class EliminarEmpresas(LoginAndActiveMixin, PermisoFletes, DeleteView):
    model = NomEmpresas
    template_name = 'fletes/catalogos/empresas/nomempresas_confirm_delete.html'
    login_url = reverse_lazy('users_app:user_login')
    ...
    def post(self, request, *args, **kwargs):
        ...
        object.activo = False
        object.save()
        return redirect('fletes:catalogos:empresas:lista')
