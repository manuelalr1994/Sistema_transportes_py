...
class RegistrarCuadrilleros(LoginAndActiveMixin, PermisoFletes, CreateView):
    model = NomCuadrilleros
    form_class = forms.FormularioCuadrilleros
    template_name = 'fletes/catalogos/cuadrilleros/registro_cuadrilleros.html'
    success_url = reverse_lazy('fletes:catalogos:cuadrilleros:lista')
...
class ListaCuadrilleros(LoginAndActiveMixin, PermisoFletes, ListView):
    model = NomCuadrilleros
    template_name = 'fletes/catalogos/cuadrilleros/cuadrilleros.html'
    login_url = reverse_lazy('users_app:user_login')
...
class ConsultarCuadrilleros(LoginAndActiveMixin, PermisoFletes, DetailView):
    model = NomCuadrilleros
    template_name = 'fletes/catalogos/cuadrilleros/consulta_cuadrilleros.html'
    login_url = reverse_lazy('users_app:user_login')
...
class ActualizarCuadrilleros(LoginAndActiveMixin, PermisoFletes, UpdateView):
    model = NomCuadrilleros
    form_class = forms.FormularioCuadrilleros
    template_name = 'fletes/catalogos/cuadrilleros/actualizar_cuadrilleros.html'
    success_url = reverse_lazy('fletes:catalogos:cuadrilleros:lista')
    login_url = reverse_lazy('users_app:user_login')


# Modulo para Inactivar Cuadrilleros
class EliminarCuadrilleros(LoginAndActiveMixin, PermisoFletes, DeleteView):
    model = NomCuadrilleros
    template_name = 'fletes/catalogos/cuadrilleros/nomcuadrilleros_confirm_delete.html'
    login_url = reverse_lazy('users_app:user_login')
    ...
    def post(self, request, *args, **kwargs):
        ...
        object.activo = False
        object.save()
        return redirect('fletes:catalogos:cuadrilleros:lista')
