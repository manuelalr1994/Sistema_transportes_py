...
from applications.nomina.catalogos.semanas.models import NomTipoSemanas, NomSemanas
    ...
    if request.user.permisos_fletes:
        ...
        if request.method=="POST":
            ...
            if tipo == None:
                ...
                contexto['exitoso'] = False
                return JsonResponse(contexto)
            elif len(request.POST.getlist('lista_fecha_inicial[]')) <= 0 or len(request.POST.getlist('lista_fecha_final[]')) <= 0 or len(request.POST.getlist('lista_id[]')) <= 0:
                error = '<p style="color: red;">Por favor, genere la lista de fechas</p>'
                contexto['error'] = error
                contexto['exitoso'] = False
                return JsonResponse(contexto)
            else:
            ...
            return render(request, 'fletes/catalogos/semanas/registro_semanas.html', contexto)
    else:
        return redirect('users_app:user_error')

class ListaTiposSemanas(LoginAndActiveMixin, PermisoFletes, View):
    model = NomTipoSemanas
    template_name = 'fletes/catalogos/semanas/lista_semanas.html'
    login_url = reverse_lazy('users_app:user_login')

    def get_context_data(self, **kwargs):
        context = {}
        tipo_buscado = self.request.GET.get('tipo_buscado')
        if tipo_buscado is not None:
            context['lista_tipos_semana'] = self.model.objects.filter(nombre__icontains=tipo_buscado)
        else:
            context['lista_tipos_semana'] = self.model.objects.all()
...
class ConsultarTiposSemanas(LoginAndActiveMixin, PermisoFletes, View):
    model = NomSemanas
    template_name = 'fletes/catalogos/semanas/consulta_semanas.html'
    login_url = reverse_lazy('users_app:user_login')
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        nombre_semana = NomTipoSemanas.objects.filter(tipo = tipo_semana).values('nombre')[0]['nombre']
        contexto['tipo'] = tipo_semana
        contexto['nombre'] = nombre_semana
        contexto['lista_semanas'] = self.model.objects.filter(tipo_semana = tipo_semana)

        for semana in contexto['lista_semanas']:
            semana.fecha_inicial = semana.fecha_inicial.strftime("%d/%m/%Y")
            semana.fecha_final = semana.fecha_final.strftime("%d/%m/%Y")
