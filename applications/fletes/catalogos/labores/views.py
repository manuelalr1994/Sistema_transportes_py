...
class RegistrarLabores(LoginAndActiveMixin, PermisoFletes, CreateView):
    model = NomLabores
    form_class = FormularioLabores
    template_name = "fletes/catalogos/labores/registro_labor.html"
    success_url = reverse_lazy('fletes:catalogos:labores:lista')
...
class ListaLabores(LoginAndActiveMixin, PermisoFletes, ListView):
    model = NomLabores
    template_name = "fletes/catalogos/labores/lista_labores.html"
    login_url = reverse_lazy('users_app:user_login')
